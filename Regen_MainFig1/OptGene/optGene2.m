function [x, population, scores, optGeneSol] = optGene2(model, targetRxn, substrateRxn, generxnList, varargin)
...
parser.addParamValue('FitnessLimit', inf, @(x) isnumeric(x)); % ADDED TO ALLOW TERMINATION AS SOON AS A GC DESIGN IS DISCOVERED
...
FitnessLimit = parser.Results.FitnessLimit; % ADDED TO ALLOW TERMINATION AS SOON AS A GC DESIGN IS DISCOVERED
...
options = gaoptimset(...
    ...
    'FitnessLimit', FitnessLimit ... % ADDED TO ALLOW TERMINATION AS SOON AS A GC DESIGN IS DISCOVERED
...
% save the solution
[optGeneSol] = GetOptGeneSol2(model, targetRxn, substrateRxn, generxnList, population, x, scores, geneok, saveFile, outputFolder); % in case of genes
